...
WORKDIR /src
COPY ["AzUpdateFunc/AzUpdateFunc.csproj", "AzUpdateFunc/"]
RUN dotnet restore "./AzUpdateFunc/AzUpdateFunc.csproj"
COPY . .
WORKDIR "/src/AzUpdateFunc"
RUN dotnet build "./AzUpdateFunc.csproj" -c $BUILD_CONFIGURATION -o /app/build


# 이 스테이지는 최종 스테이지에 복사할 서비스 프로젝트를 게시하는 데 사용됩니다.
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./AzUpdateFunc.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
